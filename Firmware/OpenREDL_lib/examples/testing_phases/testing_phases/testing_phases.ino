...
void setup() {
  ...
  storage_interval_s = 5; // Default 10 (s) 
...
diff1.gain = ADS1115_PGA_0P256;
//diff2.gain = ADS1115_PGA_6P144;
//diff3.gain = ADS1115_PGA_0P512;
diff4.gain = ADS1115_PGA_0P512;
//diff5.gain = ADS1115_PGA_6P144;
diff6.gain = ADS1115_PGA_0P512;
...
diff1.mult_factor = 80./333;
//diff2.mult_factor = 1.;
//diff3.mult_factor = 1.;
diff4.mult_factor = 0.6828;
//diff5.mult_factor = 1.;
diff6.mult_factor = 0.6828;
...
// Todo: write general in diff_channel_class line 500
void print_n_continuous_sample(byte n, int *Vout, int *Iout){
        Serial.flush();
        Serial.print(F("JSONSERIES<"));
        Serial.print(F("{\"phase1.wave.V[V]\":["));
        for (int i=0; i<n;i++){
            Serial.print(Vout[i]*1.0/32700*0.682806*512);
            if (i < n-1) Serial.print(F(","));
            Serial.flush();
        }
        Serial.print(F("], \"phase1.wave.I[A]\":["));
        for (int i=0; i<n;i++){
            Serial.print(Iout[i]*1.0/32700*80/333.*512);
            if (i < n-1) Serial.print(F(","));
            Serial.flush();
        }
        Serial.print(F("]"));

        Serial.println(F(", \"type\":\"series\"}>"));        
        Serial.flush();
}

void samples_to_take(){

//    if (count_sampling_rounds == 0){
//        byte n = 40;
//        int Vout[40], Iout[40];
//        phase1.get_n_continuous_sample(n,Vout,Iout);
//        print_n_continuous_sample(n,Vout,Iout);
//        reset_measurements();
//    }
...
// [phase].get_continuous_sample([sample_window_cycles],[grid_frequency]);
    phase1.get_continuous_sample(24,50); // phase1(I_channel = diff1, V_channel = diff4)
    //phase2.get_continuous_sample(25,50); // phase2(I_channel = diff2, V_channel = diff5)
    //phase3.get_continuous_sample(24,50); // phase3(I_channel = diff3, V_channel = diff6)
   //delay(150);
...
//#############################################################################
    ...
    add_to_log(F("changes"), F(""), float(phase1.change_of_sign),decimal_digits);
    add_to_log(F("count_sampling_rounds"), F(""), float(count_sampling_rounds),decimal_digits);
    add_to_log(F("sum_deltat_first_last_cycle_s"), F(""), phase1.sum_deltat_first_last_cycle_s,3);
    add_to_log(F("sampling_time_sum_s"), F(""), phase1.sampling_time_sum_s,3);
    add_to_log(F("f1a"), F("Hz"), phase1.calculate_fa_Hz(),3);
    add_to_log(F("mean_sampling_time"), F("Hz"), float(mean_sampling_time),3);

 
    //add_to_log(F("IV_n"), F("IV"), print_get_n_continuous_sample(n,Vout,Iout));
    //print_get_n_continuous_sample(n,Vout,Iout);
